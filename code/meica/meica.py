...
extopts.add_option('',"--daw",dest='daw',help=SUPPRESS_HELP,default='5')
...
sl.append("%s%s %s %s -e %s  -d %s --sourceTEs=%s --kdaw=%s --rdaw=0 --initcost=%s --finalcost=%s --conv=2.5e-5 %s %s" % (tedflag,sys.executable,interactive_flag, '/'.join([meicadir,tedanapath]),options.tes,ica_input,options.sourceTEs,options.daw,options.initcost,options.finalcost,strict_setting, options.ted_args))
if outprefix=='': outprefix=setname

if options.select_only: 
	sl = []
	sl.append("%s %s %s -e %s  -d %s --sourceTEs=%s --kdaw=%s --rdaw=0 --initcost=%s --finalcost=%s --conv=2.5e-5 --mix=meica_mix.1D %s %s " % (sys.executable,interactive_flag, '/'.join([meicadir,tedanapath]),options.tes,ica_input,options.sourceTEs,options.daw,options.initcost,options.finalcost,strict_setting, options.ted_args))
...
sl.append("voxsize=`ccalc .85*$(3dinfo -voxvol eBbase.nii.gz)**.33`") 
sl.append("voxdims=\"`3dinfo -adi eBbase.nii.gz` `3dinfo -adj eBbase.nii.gz` `3dinfo -adk eBbase.nii.gz`\"")
sl.append("echo $voxdims > voxdims.1D")
